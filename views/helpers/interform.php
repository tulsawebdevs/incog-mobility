 ...
 *
 */
class InterformHelper extends FormHelper {
	var $modelMap;
	...
	var	$selectorMonths = array(
		"201007"=>"July 2010","201008"=>"August 2010","201009"=>"September 2010","201010"=>"October 2010"
		);

/*
 * Overload some existing methods
 */
	function month($fieldName="selectedMonth",$selected=NULL,$attributes=array()) {
		$html=<<<EOF
		<select name={$fieldName} class="datePicker">
EOF;
		foreach ($this->selectorMonths as $val=>$label) {
			$selectedOpt = '';
			if($val==$selected) $selectedOpt = "selected=\"selected\"";
			$html.="<option $selectedOpt value=\"$val\">$label</option>\n";
		}
		$html .= "</select>";
		return $html;
	}
	...
	function create($model,$options=array(),$mode='submitting') {
		...
		$ret  = str_replace("<input","\n<input",parent::create($model,$options));
		$ret  = str_replace("<div","\n<div",$ret);
		$ret .= "<input type=\"hidden\" name=\"formDebug\" value=\"".Configure::read("debug")."\"/>";
		return  $ret;
	}
	
	function input($fieldName, $options = array(),$skipDiv=false) {
		if  (isset($this->editingModel[$this->model()])
			&&isset($this->editingModel[$this->model()][$fieldName])
			) {
...
//		}
	...
	function actions($opts=array()) {
		if(isset($opts["submitLabel"])) {
		...
		$html .= "\n\t<button class=\"primary ".strtolower($submitLabel)."\" type=\"submit\">";
		$html .=  "\n\t<img src=\"/img/icons/".strtolower($submitLabel).".png\" alt=\"$submitLabel\" />$submitLabel";
		$html .= "\n\t</button>";
//		require_once("html.php");
		$htmlHelper  = new HtmlHelper;
		if(!isset($opts["nocancel"]) || !$opts["nocancel"]) {
			$label="Cancel";
			$cancelaction = "index";
			if(isset($opts["cancelaction"])) {
				$cancelaction = $opts["cancelaction"];
			}
			if(isset($opts["cancelLabel"])) {
				$label = $opts["cancelLabel"];
			}
			$html  .= $htmlHelper->link($label,array("url"=>$cancelaction));
		}
		$html .= "\n</div>";
		return "\n".$html;
	}
	function end($options=null) {
		$html = "";
	//contains only the formModel hidden input right now. should eventually have the rest of this stuff
	//echo $interface->formInfo($Interface);
//		$tableDataController = strtolower(Inflector::pluralize($Interface->model));
		
		// was 	<?php echo $tableDataController
		$formModel = $this->model();
		//used by the toggler so should be populated with plural .. 
		$ajaxDataController =Inflector::pluralize($formModel); 
		$html .=<<<EOF
\n\n
<div id="ajaxInfo">
	<div class="trigger"></div>
	<div id="debug"></div>
	<div id="eventHandlers" data-formModel="{$formModel}">
		<input data-elementId="phone" type="hidden" class="ajaxInfo urlParam"  value="/participants/addPhoneTo/study">
	</div><!-- /#eventHandlers -->
	<div id="otherAjaxInfo">
		<input type="hidden" id="ajax-data-controller" value="{$ajaxDataController}" />
	</div><!-- /#otherAjaxInfo -->
</div><!-- /#ajaxInfo -->\n
EOF;
		
		$parentHtml = parent::end($options);
		$html .= $parentHtml;
		return $html;
	}
...
/*
 *  Name wrapper. arg can be a normal set of Cakish options, or
 * "required" or
 * a string like testFirst|testLast, which will cause the two fields to be prepopulated
 ...
 */
	function fullName($attrs=null) {
		...
		if  ($attrs=="required") {
			...
			$attrsLast = $attrs;
			if (is_scalar($attrs)) {
				if(sizeof(explode("|",$attrs)) ==2 ) {
					$exp = explode("|",$attrs);
					//pipe-delimited first-last as init value
					$attrsFirst = array_merge(array("div"=>false),array("value" => $exp[0]));
					$attrsLast = array_merge(array("div"=>false),array("value" => $exp[1]));
//				$attrsLast["value"] = $exp[1];
				}else{
...
//		pr($attrsLast); 
		...
		$html = '<div class="field">';
		$html .= $this->input("first_name",$attrsFirst,true);
		$html .= '</div>';
		$html .= '<div class="field">';
		$html .= $this->input("last_name",$attrsLast,true);
		$html .= '</div>';
		return $html;
	}
...
/*
 * AJAX-oriented form elements
 */

	//provides a text input which, when typed in, adds an xref entry
	//connecting the current editingModel to a record having the entered value
	//in the specified column
	//Example: stock_ticker_symbol
	// 		type AA and press enter; the record in the 'stocks' table
	//			with ticker_symbol='AA' will be attached to this model via ajax
	//	also includes a div that is updated with the list being built
	
 	function appender($lookupField) {
/* 		pr("appender() has editingModel:");
 		pr($this->editingModel);
*/
 		//derive information about the xref from field name
 		$e = explode("_",$lookupField);
 		$lookupField = str_replace($e[0]."_","",$lookupField);
 		$foreignModel=Inflector::pluralize($e[0]);
 		//not too excited about editingModel[User][User][id] but that's what 
 		// came from User->read(null,69)
 		$thisModelId = $this->editingModel[$this->model()]["id"];
// 		pr("field in table '$foreignModel' should be $lookupField thisModelId $thisModelId");
 		$ajaxUrl = "/".$foreignModel."/attachTo/".strtolower($this->model())."/".$thisModelId;
 		pr("built ajaxUrl $ajaxUrl");
 		$html = "";
 		$html .= $this->input("add_".$lookupField,array("class"=>"ajax","data-ajaxUrl"=>$ajaxUrl));
 		
// 		$html .=<<<EOF
//	<div id="{$foreignModel}"></div>
//EOF;
